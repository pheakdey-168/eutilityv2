<template>
    <v-row class="">
        <v-col sm="12" cols="12" class="pt-0">
            <h2 class="mb-0 font_20">{{ $t('member_fee') }}</h2>
            <v-dialog v-model="dialogm2" max-width="500px">
                ...
                <v-form ref="form" v-model="valid" lazy-validation>
                    <v-card>
                        <v-card-title>{{ $t('add_member_fee') }}</v-card-title>
                            ...
                            <v-row>
                                <v-col sm="12" cols="12" class="pb-0">
                                    ...
                                    <v-select
                                        ...
                                        return-object
                                        outlined/>
                                    <label class="label">{{ $t('revenue_account') }}</label>
                                    <v-select class="mt-1"
                                              v-model="p.revenueAccount"
                                              :items="revenueAccounts"
                                              item-value="id"
                                              :item-text="item =>`${item.number} - ${item.name}`"
                                              return-object
                                              placeholder="Account Revenue"
...
<script>
import MemberFeeModel from "@/scripts/model/MemberFee"
        ...
        async save() {
            ...
            new Promise(resolve => {
                setTimeout(() => {
                    resolve('resolved')
                    this.showLoading = true
                    loanHandler.memberFeeCreate(new MemberFeeModel(this.p)).then(response => {
                        this.showLoading = false
