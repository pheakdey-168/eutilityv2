<template>
    <v-row class="">
        <v-col sm="12" cols="12" class="pt-0">
            ...
            <!-- <p class="mb-0">{{ $t('loan_level_description') }} </p> -->
        </v-col>
        <v-col sm="12" cols="12">
            <template>
                <v-simple-table class="attachment_table">
                    <template v-slot:default>
                        ...
                        <tbody>
                        <tr v-for="loanp in loanInterests" v-bind:key="loanp.id">
                            <td>{{ loanp.name }}</td>
...
import LoanInterestModel from "@/scripts/model/credit/LoanInterest"
    ...
    methods: {
        ...
        async save() {
            ...
            new Promise(resolve => {
                setTimeout(() => {
                    resolve('resolved')
                    loanHandler.loanInterestCreate(new LoanInterestModel(this.l)).then(response => {
                        this.showLoading = false
                        if (response.data.statusCode === 201) {
                            this.close()
                            this.loadLoanInterest()
