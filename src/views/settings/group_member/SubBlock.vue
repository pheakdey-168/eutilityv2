<template>
    <v-row class="">
        <v-col sm="12" cols="12" class="pt-0">
            <h2 class="mb-0 font_20">{{ $t('district') }}</h2>
            <v-dialog v-model="dialogm2" max-width="500px">
                ...
                <v-form ref="form" v-model="valid" lazy-validation>
                    <v-card>
                        <v-card-title>{{ $t('add_sub_block') }}</v-card-title>
                        <v-icon class="btn_close" @click="dialogm2 = false">close</v-icon>
                        <v-divider/>
                        <v-card-text style="height: 220px; background-color: #EDF1F5; color: #333333;">
                            <v-row>
...
<script>
import BlockModel from "@/scripts/model/group_member/SubBlock"
        ...
        async save() {
            ...
            new Promise(resolve => {
                setTimeout(() => {
                    ...
                    loanHandler.subBlockCreate(new BlockModel(this.block)).then(response => {
