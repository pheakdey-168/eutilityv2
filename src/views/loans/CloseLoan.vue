                                            ...
                                            <v-col>
                                                <v-row>
                                                    <v-col sm="6" cols="12" class="pl-0 pb-0">
                                                        ...
                                                        <label class="label  mb-0">{{ $t('loan_account') }}</label>
                                                        <v-select
                                                            class="mt-1"
                                                            :items="loanList"
...
const $ = kendo.jQuery
import CloseLoanModel from '@/scripts/model/credit/CloseLoan'
...
export default {
    ...
    methods: {
        ...
        async loanChange() {
            this.currencyCode = this.l.loan.loanProduct.currency.code
            await this.loadTransactionRate()
            new Promise(resolve => {
                setTimeout(() => {
                    resolve('resolved')
                    this.showLoadingLoan = true
                    memberHandler.loanScheduleGet(this.l.loan.id).then(res => {
        ...
        async save() {
            ...
            new Promise(resolve => {
                setTimeout(() => {
                    resolve('resolved');
                    memberHandler.closeLoanPost(new CloseLoanModel(this.l)).then(response => {
                    ...
                    loanHandler.businessSettingGet().then(res => {
                        ...
                        if (res.data.statusCode === 200) {
                            ...
                            if(s.length > 0){
                                ...
                                this.l.cashAccount = s[0].cashAccount
                            }else{
        ...
        async getJournalNumber() {
            let num = await Helper.generateAccoutingNumber(this.journalTxnType, this.l.issuedDate)
        ...
        async addJournal(){
            ...
            this.showLoading = true
            let amt = parseFloat(obj.loan.amount) + parseFloat(obj.penalty)
            let journLine = []
            journLine.push({
                "uuid": "",
            ...
            journLine.push({
                ...
                "third_contacts": [],
                "ref_number": '',
                "amount": (parseFloat(obj.loan.amount) * -1),
                "discount": {},
                "mark_up": 0,
                "billable": 0,
                "tax_code": "",
                ...
                "exchange_rate_uuid": "",
                "exchange_rate": this.rate,
                "exchanged_amount": ((parseFloat(obj.loan.amount) * -1)*parseFloat(this.rate)),
                "tax_amount": 0,
                "exchanged_tax_amount": 0,
                "is_reconciled": 0,
                "is_settled": 0,
        ...
        interestChange(){
            if(this.l.interest > this.oldInterest || this.l.interest == ''){
