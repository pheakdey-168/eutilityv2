                            ...
                            <v-col sm="12" col="12">
                                <v-row>
                                    <v-col sm="6" cols="12" class="pb-0 pl-0">
                                        ...
                                        <app-datepicker
                                            :initialDate="disburs.issuedDate"
                                        ...
                                        <app-datepicker
                                            :initialDate="disburs.startPaymentDate"
                                            @emitDate="disburs.startPaymentDate = $event"
                                            @onChanged="principalPaidAmountChange"
                                        />
                                        <label class="label  mb-0">{{ $t('loan_term') }}</label>
                                        <v-text-field
                                            class="mt-1"
                                            type="number"
                                            style="text-align: right"
                                            v-model="loan.period"
                                            @change="principalPaidAmountChange"
                                            outlined
                                        />
                                        ...
                                        />
                                        <label class="label  mb-0">{{ $t('pay_on_principal') }}</label>
                                        <vue-numeric
                                            ...
                                            v-model="disburs.principalPaidAmount"
                                            @change="principalPaidAmountChange"
...
import LoanRescheduleModel from '@/scripts/model/credit/LoanReschedule'
export default {
    data: () => ({
        valid: true,
        showLoading: false,
    ...
    methods: {
            ...
            new Promise(resolve => {
                setTimeout(() => {
                    ...
                    memberHandler.getLoanByMemberId(this.disburs.member.id).then(res => {
                        this.showLoading = false
                        window.console.log(res.data.data, 'loan')
        ...
        async loanChange() {
            ...
            new Promise(resolve => {
                setTimeout(() => {
                    ...
                    memberHandler.loanScheduleGet(this.disburs.loan.id).then(res => {
                        this.showLoading = false
                        if (res.data.statusCode === 200) {
                            this.showLoadingLoan = false
                            let data = res.data.data
                            this.oldSchedule = data
                            let total = 0, int = 0
                            // this.scheduleAR = []
                            ...
                            this.disburs.interest = int
                            this.oldInterest = int
                            this.disburs.principal = total
                            this.principalPaidAmountChange()
                        }
                    }).catch()
                    {
                        this.showLoading = false
            ...
            this.loan.number = this.loan.number + '-' + (parseInt(this.loan.rescheduled) + 1)
        },
        //customer
        ...
        async save() {
            this.r.issuedDate = this.disburs.issuedDate
            this.r.userPin = this.pin
            this.r.newLoan = this.loan
            this.r.oldloan = this.disburs.loan
            this.r.journalId = this.journalId
            this.r.rescheduleHistories = this.disburs.loan.rescheduleHistories
            this.r.rescheduleHistories.push({
                startPaymentDate: this.disburs.startPaymentDate,
                period: this.loan.period,
                interestRatePerMonth: this.loan.interestRatePerMonth,
                principalPaidAmount: this.disburs.principalPaidAmount
            })
            this.r.principalPaidAmount = this.disburs.principalPaidAmount
            this.r.exchangeRate = this.exchangeRate
            this.r.period = this.loan.period
            this.r.interestRatePerMonth = this.loan.interestRatePerMonth
            this.r.paymentSchedule = this.scheduleAR
            this.r.member = this.disburs.member
            this.r.oldSchedule = this.oldSchedule
            this.showLoadingData = true
            memberHandler.loanRescheduleCreate(new LoanRescheduleModel(this.r)).then(response => {
        ...
        makeJournal() {
            ...
            let dr = new JournalEntryModel({
                account: this.disburs.cashAccount,
                description: '',
                primary_contact: this.disburs.member,
                amount: parseFloat(this.disburs.principalPaidAmount),
                currency: this.currency,
                exchange_rate: parseFloat(this.exchangeRate.rate), // Will change later
                exchanged_amount: parseFloat(this.disburs.principalPaidAmount) * parseFloat(this.exchangeRate.rate),
            });
            entries.push(dr);
            let drl = new JournalEntryModel({
                account: this.loan.loanProduct.loanAccount,
                description: '',
                primary_contact: this.disburs.member,
                amount: parseFloat(this.loan.amount),
                currency: this.currency,
                exchange_rate: parseFloat(this.exchangeRate.rate), // Will change later
                exchanged_amount: parseFloat(this.loan.amount) * parseFloat(this.exchangeRate.rate),
            });
            entries.push(drl);
            // Cr
            let cr = new JournalEntryModel({
                ...
                currency: this.currency,
                exchange_rate: parseFloat(this.exchangeRate.rate), // Will change later
                exchanged_amount: parseFloat(this.totalRemainning) * parseFloat(this.exchangeRate.rate),
            });
            entries.push(cr);
        ...
        async addJournal() {
            ...
            this.journal = new JournalModel({
                description: '',
                transaction_type: LoanType.RESCHEDULE,
                journal_type: LoanType.RESCHEDULE,
            });
            this.journal.created_by = this.loggedUser
            this.journal.journal_date = Helper.toISODate(this.disburs.issuedDate)
            ...
            journalHandler.save(new JournalModel(this.journal))
                .then(function (response) {
                    ...
                    self.journalId = response.data.uuid
                    self.save()
                })
        ...
        principalPaidAmountChange(){
            if(Object.keys(this.disburs.loan).length == 0){
                return
            }
            let total = parseFloat(this.totalRemainning) - parseFloat(this.disburs.principalPaidAmount)
            this.loan.amount = total
            if(total >= 0){
